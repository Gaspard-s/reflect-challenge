...
def process_departments(departments: List[Dict[str, Any]]) -> pd.DataFrame:
    """
    Processus de transformation des données des départements.

    Args:
        departments (List[Dict[str, Any]]): Liste des départements.

    Returns:
        pd.DataFrame: DataFrame contenant les données des départements.
    """
    if not departments:
        return pd.DataFrame()
...
def transform_user_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Supprimer la colonne 'applicationData' après transformation
    df.drop(columns=['applicationData'], inplace=True)
